...
default font size 12. Implements requirements 1-55.
"""
...
class PEMSAnalysisGUI(object):
    def __init__(self, root):
        ...
        self.report_format_path = tk.StringVar()
        ...
        # Alignment items shown (no mapping applied by default; per step 55)
        ...
        self.data_format_listbox.selection_set(0)  # Select 'EPA PEMS'
        ...
        # 20, 21) Alignment & Report checkboxes + Report Format entry (horizontal)
        ...
        lbl_report_fmt = tk.Label(self.frame_checks_mid, text="Report Format:", font=self.font_default, bg="#f3f3f3")
        lbl_report_fmt.grid(row=0, column=2, sticky="e")

        self.ent_report_format = tk.Entry(self.frame_checks_mid, textvariable=self.report_format_path,
                                          font=self.font_default)
        self.ent_report_format.grid(row=0, column=3, sticky="ew", padx=(6, 6))

        self.btn_format = tk.Button(self.frame_checks_mid, image=(self.icon_PEMSreport or self.icon_read),
                                    command=self.select_report_format, bd=0, bg="#f3f3f3",
                                    activebackground="#e5e5e5", cursor="hand2")
        self.btn_format.grid(row=0, column=4, sticky="w")
        self.create_tooltip(self.btn_format, "Select report format Excel")
        self.frame_checks_mid.columnconfigure(3, weight=1)
        ...
        # 52) Options/Controls and Import folder button
        self.frame_opts_top = tk.Frame(self.tab_opts, bg="#f3f3f3")
        self.frame_opts_top.pack(fill="x", padx=20, pady=(15, 5))

        lbl_opt = tk.Label(self.frame_opts_top, text="Options/Controls:", font=self.font_default, bg="#f3f3f3")
        ...
        tk.Label(self.frame_opts_list, text="Options Lists", font=self.font_header, bg="#f3f3f3").pack(anchor="w")
        ...
        self.options_listbox = tk.Listbox(opt_container, height=5, font=self.font_default,
        ...
        tk.Label(self.frame_report_list, text="Report PDF", font=self.font_header, bg="#f3f3f3").pack(anchor="w")
        ...
        self.reportPMS_listbox = tk.Listbox(rep_container, height=10, font=self.font_default,
                                            ...
                                            exportselection=False, borderwidth=1, relief="solid")
        for item in ['PEMS Default', 'PEMS LD Emissions', 'PEMS LD Quality',
                     'PEMS HD Emissions', 'PEMS HD Quality', 'PEMS HD Off-cycle Bins',
                     'Horiba Dilute', 'Custom']:
            self.reportPMS_listbox.insert(tk.END, item)
        self.reportPMS_listbox.selection_set(0)  # Default select 'PEMS Default'
    ...
    def position_obd_plot_and_pems_entry(self):
        ...
        try:
            ...
            # Adjust PEMS Entry width tightly before the Folder button
            ...
            self.ent_pems.config(width=width_chars_pems)
    ...
    def select_report_format(self):
        """Select the Report Format Excel file and set 'PEMS Default' in Report PDF list."""
        file = filedialog.askopenfilename(title="Select Report Format Excel",
                                          filetypes=[("Excel files", "*.xlsx *.xls")])
        if file:
            self.report_format_path.set(file)
            # Select 'PEMS Default' in report listbox
            try:
                items = self.reportPMS_listbox.get(0, tk.END)
                idx = items.index('PEMS Default')
                self.reportPMS_listbox.selection_clear(0, tk.END)
                self.reportPMS_listbox.selection_set(idx)
            except Exception:
                pass
    ...
    def select_obd_file(self):
        ...
        try:
            ...
            # Ensure eTIME after reading (from sTIME if available)
            tmp = ensure_eTIME(tmp)

            # Insert eTIME label rows BEFORE slicing if missing
            if 'eTIME' not in tmp.columns:
                tmp.insert(1, 'eTIME', None)
                tmp.loc[0, 'eTIME'] = 'eTIME'
                tmp.loc[1, 'eTIME'] = 's'

            # Before assigning rows, create summary starting from "Summary Information:"
            try:
                ...
                # Data rows between header rows and summary
                tmp = tmp.loc[2:summary_row_idx-1, :].copy()
            except Exception:
                # No summary region found; keep data rows from 2 onward
                tmp = tmp.loc[2:, :].copy()
    ...
    def open_option_item(self, event):
        ...
        try:
            if item.lower().endswith((".xlsx", ".xls")) and ("rde_settings_" in item.lower()):
                ...
                # .mlx is a zipped MATLAB Live Script; store path or implement parser if available
    ...
    def read_file(self):
        ...
        - Ensure eTIME in seconds from sTIME using ensure_eTIME()
        ...
        try:
            ...
            try:
                ...
                # Data rows between headers and summary
            ...
            # Populate Data Fields listbox with original header labels (columns_row)
    ...
    def check_alignment(self):
        ...
        - Use selected item in align_listbox directly as align_col (per step 55)
    ...
    def plot_check_alignment(self, pems_path, obd_path, pems_df, align_col):
        ...
        # Close any open figures before opening the interactive window
        if plt.get_fignums():
            plt.close('all')
    ...
    def run_analysis(self):
        ...
        # Print index of 'Vehicle Speed' in df.columns if exists
