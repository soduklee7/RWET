...
# os.chdir("C:/RDE")
eTIME_DATA_PLOT = 2000
eTIME_DATA_PLOT_flag = False
VEH_SPD_UNIT = 'mph'
engine_cold_start_ECT = 70.1
...
T2Bin5_NOx, T2Bin5_NMOG, T2Bin5_CO, T2Bin5_HCHO, T2Bin5_PM = 0.07, 0.09, 4.2, 0.018, 0.01
...
'''        
def vehmove_start_end(df, vehmove_start_ini, Inst_Mass_GHG, GHG_WLTC_threshold):
    
    nel = len(df)
           
    # num_stops = 0
    # for i in range(vehmove_start_ini+1, nel):
    #     if df['vehspd_ecu'][i] > 0.5 and df['vehspd_ecu'][i-1] <= 0.01 and np.sum(Inst_Mass_GHG[i:nel-1]) > GHG_WLTC_threshold:
    ...
    # j = k =0
    # for i in range(vehmove_start_ini+1, nel):
    #     if df['vehspd_ecu'][i] > 0 and df['vehspd_ecu'][i-1] <= 0 and (np.sum(Inst_Mass_GHG[i:]) > GHG_WLTC_threshold):
    #         vehmove_start[j]=i
    #         j = j + 1
    #     elif df['vehspd_ecu'][i] <= 0 and df['vehspd_ecu'][i-1] > 0 and (np.sum(Inst_Mass_GHG[i:]) > GHG_WLTC_threshold):
    #         vehmove_end[k]=i-1
    #         k = k+1
                        
    # if max(vehmove_start) > max(vehmove_end):
    #     for m in range (int(max(vehmove_start))+1, nel-1):
    #         if sum(Inst_Mass_GHG[m:]) < GHG_WLTC_threshold and df['vehspd_ecu'][m] > 0:
    #             vehmove_end[k]=m-1
    #             break
            
    # df['nstop'] = np.zeros(len(df))
    # vehmove_start  = np.where(df.loc[(df['vehspd_ecu'] > 0) & (df['vehspd_ecu'].shift(1) <= 0) & (Inst_Mass_GHG.iloc[::-1].cumsum() > GHG_WLTC_threshold), 'nstop'], df.loc[(df['vehspd_ecu'] > 0.5) & (df['vehspd_ecu'].shift(1) <= 0) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] > GHG_WLTC_threshold), 'nstop'].index, 0)
    # # vehmove_start  = np.where(df.loc[(df['vehspd_ecu'] > 0) & (df['vehspd_ecu'].shift(1) <= 0) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] > GHG_WLTC_threshold), 'nstop'], df.loc[(df['vehspd_ecu'] > 0.5) & (df['vehspd_ecu'].shift(1) <= 0) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] > GHG_WLTC_threshold), 'nstop'].index, 0)
    # vehmove_end  = np.where(df.loc[(df['vehspd_ecu'] <= 0) & (df['vehspd_ecu'].shift(1) > 0.5) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] > GHG_WLTC_threshold), 'nstop'], df.loc[(df['vehspd_ecu'] <= 0) & (df['vehspd_ecu'].shift(1) > 0.5) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] > GHG_WLTC_threshold), 'nstop'].index-1, 0)
    # vehmove_end_pt = np.where(df.loc[(df.index > vehmove_start[-1]) & (df['vehspd_ecu'] > 0) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] < GHG_WLTC_threshold), 'nstop'], df.loc[(df['vehspd_ecu'] > 0) & (Inst_Mass_GHG.iloc[::-1].cumsum().iloc[::-1] < GHG_WLTC_threshold), 'nstop'].index-1, 0)
    # if len(vehmove_end_pt) > 0: vehmove_end = np.concatenate((vehmove_end, vehmove_end_pt[0]), axis=None)

    # num_vehmove_start = len(vehmove_start)-1
    

    j = k = 0
    vehmove_start_kph = 0.3
    vehmove_stop_kph = 0.01
    vehmove_start_min = 50
    vehmove_start = []
    vehmove_end = []
    for i in range(vehmove_start_ini+1, nel):
        if (df['vehspd_ecu'][i] > vehmove_start_kph) and (df['vehspd_ecu'][i-1]) <= vehmove_stop_kph and np.sum(Inst_Mass_GHG[i:-1]) > GHG_WLTC_threshold:
            if (i > vehmove_start_min) and ((len(vehmove_start) == 0) or (len(vehmove_start) == len(vehmove_end))): 
            ...
            if (i > j) and len(vehmove_end) < len(vehmove_start): 
                if (i - j) < vehmove_start_min: 
                    vehmove_end.append(i+vehmove_start_min)
                else:
    ...
    if (len(vehmove_end) < len(vehmove_start)):
        ...
        if len(_idx_ends) > 0: 
            ...
            vehmove_end = np.concatenate((vehmove_end, vehmove_start[-1] + 300), axis=None)
...
def df_resample_by_second(df, time_column, time_resampling, EPA_PEMS_columns_units):
    ...
    for i, text in enumerate(EPA_PEMS_columns_units.keys()):
        ...
        if _unit in mass_flowrate_units:
            df_new[text] = pd.DataFrame(f_linear_mass_flows(_time_by_resampling))*(factors)
        else:
...
if FEV_to_EPA_PEMS: 
    # FEV_to_EPA_PEMS = TRUE
    # Read the mapping file
    mapping_sheet = 'FEV_to_EPA_PEMS_map'
    # df_mapping = pd.read_csv(mapping_file, encoding='windows-1252')
...
iCOOL_TEMP_cname = "iCOOL_TEMP" if ("iCOOL_TEMP" in df.columns) else ''
...
df['eng_Combustion_ON'] = eng_Combustion_ON = np.where(df[iENG_SPEED_cname].astype(float) >= 500, 1, 0)
...
if (not FEV_to_EPA_PEMS):    
    ...
    if vehspd_move_start > 0: 
        ...
        df = df.reset_index(drop = True)
    print('len(df) = at line 1389', len(df))        

    US_max_vehspd_mph_cutoff = 1.60934 * US_max_vehspd_kph
    vehspd_move_stop = df.loc[(df['vehspd_ecu'].shift(10) <= 0.01) & (df['vehspd_ecu'].shift(1) <= 0.1) & (df['vehspd_ecu'] > 0.5) &  (df['vehspd_ecu'].shift(-15)  > US_max_vehspd_mph_cutoff)].index
...
nel = len(df)
print('len(df) = at line 1370', len(df))
...
CatalystTemp = df[CatalystTemp_cname] if (CatalystTemp_cname != '') and (EPA_PEMS_columns_units.get(CatalystTemp_cname) == 'deg C') else 5/9 *  (df[CatalystTemp_cname] - 32)
...
if (_index_ECT_cold_start_end == len(df[ECT_cname]) -1) or (iENG_SPEED_cname == ''):
    ...
    time_engine_idle_int = 0
    cold_start_end = 1
    _index_ECT_cold_start_end = 300

if (EPA_PEMS_columns_units.get(Tamb_cname) != 'deg C'): Tamb = 5/9*(Tamb - 32)
time_engine_start = etime[df.loc[(df['eng_Combustion_ON'].shift(-1) == 1) & (df['eng_Combustion_ON'] == 0)].index[0]]
ECT_engine_start = df.loc[(df['eng_Combustion_ON'].shift(-1) == 1) & (df['eng_Combustion_ON'] == 0), ECT_cname].values[0]
_index_time_engine_start = df.loc[(df['eng_Combustion_ON'].shift(-1) == 1) & (df['eng_Combustion_ON'] == 0), 'sTIME'].index[0]
...
vehmove_start = df.loc[(vehspd_ecu.shift(-1) >= 0.5) & (vehspd_ecu < 0.1), 'sTIME']
vehmove_start_ini = vehmove_start.index[0] if len(vehmove_start) > 0 else 0

_index_time_engine_idle = df.loc[(vehspd_ecu > 0.5) & (vehspd_ecu.shift(-1) <= 0.1) & (df['eng_Combustion_ON'] == 1), 'sTIME']
...
nVm = df.loc[vehspd_ecu > vehspd_rural_kph].shape[0] # motoway driving above 90 kph
...
if (iCOOL_TEMP_cname == '') or (max(df[ECT_cname]) == 0): _index_ECT_cold_start_end = 300
...
_idx_Vm = df.loc[(df['vehspd_ecu'] > vehspd_rural_kph)].index
if len(_idx_Vm) > 0:
...
[vehmove_start, vehmove_end, num_vehmove_start] = vehmove_start_end(df, vehmove_start_ini, Inst_Mass_GHG, GHG_WLTC_threshold)
...
if (max(Altitude) > 0.01): # or (not FEV_to_EPA_PEMS):
    rgrade = np.zeros(nel)
    slope = np.zeros(nel)
...
# else:
#     rms_road_grade_savgol_pos =  'EPA RMS Road Grade 0%'
...
else: motorway_speed_above_100kph = round(num_vehspd_100kph/60*100, 2) # 60, 2)
...
if _index_time_engine_start > _index_ECT_cold_start_end and _index_ECT_cold_start_end == 300: 
