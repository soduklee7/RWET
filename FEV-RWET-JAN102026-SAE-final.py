...
urban_CO_gpmi = 1000 * np.sum(COu)/urban_distance_km * 1.60934
...
emission_extreme_extended_conditions_adjustment = False # emissions calculations (except CO2) at line 2050 part, line 2760 for labels
...
max_grade_pct = 5.0
extreme_Tamb_C = 35
extreme_Tamb_low_C = -7
extreme_ALT_m = 1300
extreme_ALT_low_m = 700
extended_Tamb_C = 30
extended_Tamb_low_C = 0
extended_ALT_m = 700
emissions_extreme_conditions_CF = 0
emissions_extended_conditions_CF = 1.6
...
def RDE_report (rpt, filename_pems, filename_obd, output_folder, now_hms, suffix, trip_distance, trip_duration, cold_start_duration, urban_distance, rural_distance, motorway_distance,
                ...
                FE_mpg, CO2_gpm_test, engNOx_mgpm_test, CO_gpm_test):
...
def RWE_report(rpt, filename_pems, filename_obd, output_folder, now_hms, suffix, trip_distance, trip_duration, cold_start_duration, urban_distance, rural_distance, motorway_distance,
                ...
                CO2_gpm_test, engNOx_mgpm_test, engNMHC_mgpm_test, CO_gpm_test, GHG_WLTC_threshold, extreme_conditions_pct, extended_conditions_pct, GHG_RDE_gpmi, CO_RDE_gpmi, NOx_RDE_mgpmi, NMHC_RDE_gpmi,
                GHG_RDEu_gpmi, CO_RDEu_gpmi, NOx_RDEu_gpmi, NMHC_RDEu_gpmi):
    ...
    CO2_RDE_gpmi, CO2_RDEu_gpmi, CO_RDE_mgpmi, CO_RDEu_mgpmi = 1.60934*CO2_RDE_mgpkm/1000, 1.60934*CO2_RDEu_mgpkm/1000, 1.60934*CO_RDE_mgpkm, 1.60934*CO_RDEu_mgpkm
    NOx_RDE_mgpmi, NOx_RDEu_mgpmi, NMHC_RDE_mgpmi, NMHC_RDEu_mgpmi = 1.60934*NOx_RDE_mgpkm, 1.60934*NOx_RDEu_mgpkm, 1.60934*NMHC_RDE_mgpkm, 1.60934*NMHC_RDEu_mgpkm
    
    GHG_RDE_gpkm, CO_RDE_gpkm, NOx_RDE_mgpkm, NMHC_RDE_gpkm = 1.60934*GHG_RDE_gpmi, 1.60934*CO_RDE_gpmi, 1.60934*NOx_RDE_mgpmi, 1.60934*NMHC_RDE_gpmi
    GHG_RDEu_gpkm, CO_RDEu_gpkm, NOx_RDEu_gpkm, NMHC_RDEu_gpkm = 1.60934*GHG_RDEu_gpmi, 1.60934*CO_RDEu_gpmi, 1.60934*NOx_RDEu_gpmi, 1.60934*NMHC_RDEu_gpmi
    _values = [FE_mpg, CO2_gpm_test, CO2_RDE_gpmi, CO2_RDEu_gpmi, CO_gpm_test, CO_RDE_mgpmi, CO_RDEu_mgpmi, engNOx_mgpm_test, NOx_RDE_mgpmi, NOx_RDEu_mgpmi, engNMHC_mgpm_test, NMHC_RDE_mgpmi, NMHC_RDEu_mgpmi]
    ...
    _strings = ['Total trip distance', 'Total trip duration', 'Cold start duration', 'Data in extreme conditions', 'Data in extended conditions', 'Urban distance', 'Rural distance', 'Motorway distance', 'Urban distance share', 'Rural distance share', 'Motorway distance share', 
                'Urban average speed']
    _values = [trip_distance, trip_duration, cold_start_duration,  extreme_conditions_pct, extended_conditions_pct, urban_distance, rural_distance, motorway_distance, urban_distance_share, rural_distance_share, motorway_distance_share, urban_avg_speed] 
    for i in range(len(_strings)):
        mask = rpt['DATA'].str.contains(_strings[i], case=False)
        _index = np.where(mask)[0].tolist()
        rpt.loc[_index[0], 'RWE Results'] = np.round(_values[i], 2)
...
def read_FEV_PEMS(df, pp_files, fev_to_epa_var, epa_units, fev_units):
    for i in range (len(pp_files)):
        ...
        if i == 0:            
            ...
            if 'PIDA6_VehicleOdometerReading' in df.columns:
                total_dist_miles = (max(df['PIDA6_VehicleOdometerReading']) - min(df['PIDA6_VehicleOdometerReading'])) * 0.000621371
            elif 'VEL_VHCL' in df.columns:
                total_dist_miles = np.sum(df['VEL_VHCL']*dt/3.6)*0.000621371
            CO2_gpm_test = np.sum(df['MF_CO2_EGH_TP'])*dt / total_dist_miles
            engNOx_mgpm_test = np.sum(df['MF_NOX_EGH_TP'])*dt / total_dist_miles
            engNMHC_mgpm_test = np.sum(df['MF_NMHC_EGH_TP'])*dt / total_dist_miles
            CO_gpm_test = np.sum(df['MF_CO_EGH_TP'])*(dt/1000) / total_dist_miles
    ...
    return [df, EPA_PEMS_columns_units, vehicle_title, pp_file, df, df2, CO2_gpm_test, engNOx_mgpm_test, engNMHC_mgpm_test, CO_gpm_test]
...
FTP3_NOx_comp_gpmi = float(df['NOx_FTP3bags_gpmi'][3])
FTP3_CO_comp_gpmi = float(df['CO_FTP3bags_gpmi'][3])
...
if FEV_to_EPA_PEMS: 
    [df, EPA_PEMS_columns_units, vehicle_title, pp_file, df1, df2, CO2_gpm_test, engNOx_mgpm_test, engNMHC_mgpm_test, CO_gpm_test] = read_FEV_PEMS(df, pp_files, fev_to_epa_var, epa_units, fev_units)
else:
...
if iGPS_ALT_cname != '':
    _idx = df.loc[df[iGPS_ALT_cname].astype(str) == 'nan'].index
    if len(_idx) > 0:
        for i in _idx:
            if i == 0:
                df[iGPS_ALT_cname][i] = df[iGPS_ALT_cname][i+1]
            elif i == len(df)-1:
                df[iGPS_ALT_cname][i] = df[iGPS_ALT_cname][i-1]
            else:
                df[iGPS_ALT_cname][i] = 0.5 * (df[iGPS_ALT_cname][i-1] + df[iGPS_ALT_cname][i+1])
...
if (max(Altitude) > 0.01): 
    ...
    if iGPS_ALT_cname != '': df[iGPS_ALT_cname] = Altitude
else:
...
if (iGPS_ALT_cname != ''):
    dAltitude = np.where(df['vehspd_ecu'] > MAW_vehspd_1_kph, df[iGPS_ALT_cname] - df[iGPS_ALT_cname].shift(1), 0)
else:
    dAltitude = Altitude = np.zeros(nel)

_idx_extreme_Tamb = Tamb[(Tamb > extreme_Tamb_C) | ( Tamb < extreme_Tamb_low_C)].index if len(Tamb[(Tamb > extreme_Tamb_C) | ( Tamb < extreme_Tamb_low_C)]) > 0 else []
_idx_extreme_ALT = Altitude[(Altitude > extreme_ALT_m)].index if len(Altitude[(Altitude > extreme_ALT_m)]) > 0 else []

num_extreme_Tamb = len(_idx_extreme_Tamb)
num_extreme_ALT = len(_idx_extreme_ALT)
extreme_conditions_pct = round((num_extreme_Tamb + num_extreme_ALT)/len(df) * 100, 1)
# df = df.drop(_idx_extreme_Tamb)
# df = df.drop(_idx_extreme_ALT)
# df = df.reset_index(drop = True)
# nel = len(df)

_idx_extended_Tamb = Tamb[(Tamb >= extreme_Tamb_C-5) & (Tamb <= extreme_Tamb_C) | ( Tamb >= extreme_Tamb_low_C) &  (Tamb <= extreme_Tamb_low_C+7)].index if len(Tamb[(Tamb >= extreme_Tamb_C-5) & (Tamb <= extreme_Tamb_C) | ( Tamb >= extreme_Tamb_low_C) &  (Tamb <= extreme_Tamb_low_C+7)]) > 0 else []
_idx_extended_ALT = Altitude[(Altitude >= extreme_ALT_low_m) & (Altitude <= extreme_ALT_m)].index if len(Altitude[(Altitude >= extreme_ALT_low_m) & (Altitude <= extreme_ALT_m)]) > 0 else []

num_extended_Tamb = len(_idx_extended_Tamb)
num_extended_ALT = len(_idx_extended_ALT)
extended_conditions_pct = round((num_extended_Tamb + num_extended_ALT)/len(df) * 100, 1)
...
########### Exclude extreme conditions for MAW emissions calculation except CO2
########### MAW emissions except CO2 divided by 1.6 CF at extended conditions
...
for i in range(0, num_vehmove_start):
    ...
    for iel in range(k, l+1):
        ...
        for jel in range (start, nel): # nel):      
            if df['vehspd_ecu'][jel] >= MAW_vehspd_1_kph: #  and jel >= _index_ECT_cold_start_end:
                total_Inst_Mass_GHG = total_Inst_Mass_GHG + Inst_Mass_GHG[jel]
                MAW_dVMT = MAW_dVMT + df['dVMT'][jel]
                if emission_extreme_extended_conditions_adjustment:
                    if (jel in _idx_extreme_Tamb) or (jel in _idx_extreme_ALT):
                        total_Inst_Mass_CO = total_Inst_Mass_CO + Inst_Mass_CO[jel]*emissions_extreme_conditions_CF
                        total_Inst_Mass_NOx = total_Inst_Mass_NOx + Inst_Mass_NOx[jel]*emissions_extreme_conditions_CF
                        total_Inst_Mass_NMHC = total_Inst_Mass_NMHC + Inst_Mass_NMHC[jel]*emissions_extreme_conditions_CF
                    elif (jel in _idx_extended_Tamb) or (jel in _idx_extended_ALT):
                        total_Inst_Mass_CO = total_Inst_Mass_CO + Inst_Mass_CO[jel]/emissions_extended_conditions_CF
                        total_Inst_Mass_NOx = total_Inst_Mass_NOx + Inst_Mass_NOx[jel]/emissions_extended_conditions_CF
                        total_Inst_Mass_NMHC = total_Inst_Mass_NMHC + Inst_Mass_NMHC[jel]/emissions_extended_conditions_CF
                else:
                ...
                if total_Inst_Mass_GHG >= GHG_WLTC_threshold :
                    ...
                    # if (emission_extreme_extended_conditions_adjustment):
                    #     if (iel in _idx_extreme_Tamb) or (iel in _idx_extreme_ALT):
                    #         CO_MAW[iel] = total_Inst_Mass_CO*emissions_extreme_conditions_CF
                    #         NOx_MAW[iel] = total_Inst_Mass_NOx*emissions_extreme_conditions_CF
                    #         NMHC_MAW[iel] = total_Inst_Mass_NMHC*emissions_extreme_conditions_CF
                    #     elif (iel in _idx_extended_Tamb) or (iel in _idx_extended_ALT):
                    #         CO_MAW[iel] = total_Inst_Mass_CO/emissions_extended_conditions_CF
                    #         NOx_MAW[iel] = total_Inst_Mass_NOx/emissions_extended_conditions_CF
                    #         NMHC_MAW[iel] = total_Inst_Mass_NMHC/emissions_extended_conditions_CF                 
                    # else:        
        ...
        # elif np.sum(df['vehspd_ecu'][start:end]) > 0:  # iel >= _index_ECT_cold_start_end and
        
        # tMAW_vehspd[kel] = np.sum(df['vehspd_ecu'][start:end])/(end-start+1) # -GHG_exclude_pts[iel]+1)
        # tMAW_engRPM[kel] = np.sum(engine_RPM[start:end])/(end-start+1) # -GHG_exclude_pts[iel]+1)
        # tMAW_ECT[kel] = np.sum(df.loc[start:end, ECT_cname])/(end-start+1) # -GHG_exclude_pts[iel]+1)
        # tMAW_Texh[kel] = np.sum(Texh[start:end])/(end-start+1) # -GHG_exclude_pts[iel]+1)
        
        tMAW_vehspd[kel] = np.mean(df['vehspd_ecu'][start:end])
        tMAW_engRPM[kel] = np.mean(engine_RPM[start:end])
        tMAW_ECT[kel] = np.mean(df.loc[start:end, ECT_cname])
        tMAW_Texh[kel] = np.mean(Texh[start:end])
        ...
        if abs(MAW_dVMT -  np.sum(df.loc[start:end, 'dVMT']))/MAW_dVMT*100 > 0.5:
            print('Difference in dVMT for MAW calculation greater than 1%')
            print('kel: ', kel, 'MAW_dVMT = ', MAW_dVMT, ', sum dVMT = ', np.sum(df.loc[start:end, 'dVMT']))
            # sys.exit(0)
        # tMAW_GHG_g[kel] = total_Inst_Mass_GHG
        kel += 1
        MAW_sizes.append(end - start + 1)
...
num_MAWu_vehspd = len(MAW_vehspd[(MAW_vehspd >= MAW_vehspd_1_kph) & (MAW_vehspd <= MAW_urban_vehspd_kph)])
num_MAWr_vehspd = len(MAW_vehspd[(MAW_vehspd > MAW_urban_vehspd_kph) & (MAW_vehspd <= MAW_rural_vehspd_kph)])
num_MAWm_vehspd = len(MAW_vehspd[(MAW_vehspd > MAW_rural_vehspd_kph) & (MAW_vehspd <= MAW_motorway_vehspd_kph)])

MAWu_vehspd = np.zeros(num_MAWu_vehspd)
MAWu_GHG_gpkm = np.zeros(num_MAWu_vehspd)
MAWu_CO_gpkm = np.zeros(num_MAWu_vehspd)
MAWu_NOx_gpkm = np.zeros(num_MAWu_vehspd)
MAWu_NMHC_gpkm = np.zeros(num_MAWu_vehspd)

MAWr_vehspd = np.zeros(num_MAWr_vehspd)
MAWr_GHG_gpkm = np.zeros(num_MAWr_vehspd)
MAWr_CO_gpkm = np.zeros(num_MAWr_vehspd)
MAWr_NOx_gpkm = np.zeros(num_MAWr_vehspd)
MAWr_NMHC_gpkm = np.zeros(num_MAWr_vehspd)

MAWm_vehspd = np.zeros(num_MAWm_vehspd)
MAWm_GHG_gpkm = np.zeros(num_MAWm_vehspd)
MAWm_CO_gpkm = np.zeros(num_MAWm_vehspd)
MAWm_NOx_gpkm = np.zeros(num_MAWm_vehspd)
MAWm_NMHC_gpkm = np.zeros(num_MAWm_vehspd)

MAW_sizes = np.array(MAW_sizes)
print("AVerage MAW size (number of points): ", int(np.mean(MAW_sizes)))
num_MAWu_windows = int(np.mean(MAW_sizes[(MAW_vehspd >= MAW_vehspd_1_kph) & (MAW_vehspd <= MAW_urban_vehspd_kph)]))
num_MAWr_windows = int(np.mean(MAW_sizes[(MAW_vehspd > MAW_urban_vehspd_kph) & (MAW_vehspd <= MAW_rural_vehspd_kph)]))
num_MAWm_windows = int(np.mean(MAW_sizes[(MAW_vehspd > MAW_rural_vehspd_kph) & (MAW_vehspd <= MAW_motorway_vehspd_kph)]))
print("AVerage urban MAW windows): ", num_MAWu_windows)
print("AVerage rural MAW windows: ", num_MAWr_windows)
print("AVerage motorway MAW windows: ", num_MAWm_windows)

j = nMAWu = nMAWr = nMAWm = 0 
for i in range(num_MAW_vehspd):
    if (MAW_vehspd[i] >= MAW_vehspd_1_kph) and (MAW_vehspd[i] <= MAW_urban_vehspd_kph):
        ...
        nMAWr = nMAWr + 1
    elif (MAW_vehspd[i] > MAW_rural_vehspd_kph) and (MAW_vehspd[i] <= MAW_motorway_vehspd_kph):
...
# 	if df['vehspd_ecu'][k] <= MAW_vehspd_1_kph: j = j + 1 # for EMROAD zero speed definition
# nstop_during_coldstart = j

# cold_stop_time = round(nstop_during_coldstart)
cold_stop_time = df.loc[(df['vehspd_ecu']<= MAW_vehspd_1_kph) & (df.index < _index_ECT_cold_start_end), 'etime'].count()
...
CO2_RDE_mgpkm = 1000*np.mean(MAW_GHG_gpkm)
CO_RDE_mgpkm = 1000*np.mean(MAW_CO_gpkm)
NOx_RDE_mgpkm = 1000*np.mean(MAW_NOx_gpkm)
NMHC_RDE_mgpkm = 1000*np.mean(MAW_NMHC_gpkm)

# MAWu_vehspd
CO2_RDEu_mgpkm = 1000*np.mean(MAWu_GHG_gpkm)
CO_RDEu_mgpkm = 1000*np.mean(MAWu_CO_gpkm)
NOx_RDEu_mgpkm = 1000*np.mean(MAWu_NOx_gpkm)
NMHC_RDEu_mgpkm = 1000*np.mean(MAWu_NMHC_gpkm)
...
if not FEV_to_EPA_PEMS: 
    CO_gpm_test = 1000 * np.sum(Inst_Mass_CO) / total_dist_miles
...
label_CO_test = str(round(CO_gpm_test/1000,1)) + " g/mile"
...
nMAWu_out = len(MAW_TOL_outside[(MAW_TOL_outside > 0) & (MAW_vehspd >= vehspd_1_kph) & (MAW_vehspd <= MAW_urban_vehspd_kph)])
nMAWr_out = len(MAW_TOL_outside[(MAW_TOL_outside > 0) & (MAW_vehspd > MAW_urban_vehspd_kph) & (MAW_vehspd <= MAW_rural_vehspd_kph)])
nMAWm_out = len(MAW_TOL_outside[(MAW_TOL_outside > 0) & (MAW_vehspd > MAW_rural_vehspd_kph) & (MAW_vehspd <= vehspd_motorway_kph)])

########## RDE Results ##########
GHG_RDE_gpmi = np.sum(Inst_Mass_GHG[(vehspd_ecu >= vehspd_1_kph)])/(df['VMT'][nel-1]/1000)*1.6034
CO_RDE_gpmi = np.sum(Inst_Mass_CO)/(df['VMT'][nel-1]/1000)*1.6034
NOx_RDE_mgpmi = 1000 * np.sum(Inst_Mass_NOx)/(df['VMT'][nel-1]/1000)*1.6034
NMHC_RDE_gpmi = np.sum(Inst_Mass_NMHC)/(df['VMT'][nel-1]/1000)*1.6034

GHG_RDEu_gpmi = np.sum(Inst_Mass_GHG[(vehspd_ecu >= vehspd_1_kph) & (vehspd_ecu <= vehspd_urban_kph)])/(np.sum(df.loc[(vehspd_ecu >= vehspd_1_kph) & (vehspd_ecu <= vehspd_urban_kph), 'dVMT'])/1000)*1.6034
CO_RDEu_gpmi = np.sum(Inst_Mass_NOx[vehspd_ecu <= vehspd_urban_kph])/(np.sum(df.loc[(df['vehspd_ecu'] <= vehspd_urban_kph), 'dVMT'])/1000)*1.6034
...
NMHC_RDEu_gpmi = np.sum(Inst_Mass_NMHC[vehspd_ecu <= vehspd_urban_kph])/(np.sum(df.loc[(df['vehspd_ecu'] <= vehspd_urban_kph), 'dVMT'])/1000)*1.6034
...
# for row_num in range(56, 70):
#     for col_num in range(0, 5):
#         worksheet.write(row_num, col_num, "Yellow Fill", yellow_fill)
    # worksheet.write(row_num, 3, "Magenta Fill", magenta_format)
    
# for row in worksheet['A56:E70']:
#     for cell in row:
#         # cell.value = 'Range D4:E5'
...
plt.scatter(sMAW_Vu, sMAWu_GHG_gpkm, s=200, linewidth=3, marker="s", facecolors='none', edgecolors='b', label = 'Urban Mean CO2: ' + str(round(1.60934*sMAWu_GHG_gpkm, 1)) + ' g/mile')
plt.scatter(sMAW_Vr, sMAWr_GHG_gpkm, s=250, linewidth=3, marker="<", facecolors='none', edgecolors='r', label = 'Rural Mean CO2: ' + str(round(1.60934*sMAWr_GHG_gpkm, 1)) + ' g/mile')
plt.scatter(sMAW_Vm, sMAWm_GHG_gpkm, s=250, linewidth=3, marker="^", facecolors='none', edgecolors='g', label = 'Motorway Mean CO2: ' + str(round(1.60934*sMAWm_GHG_gpkm, 1)) + ' g/mile')
...
NMHC = Inst_Mass_NMHC * np.logical_and(vehspd_ecu >= MAW_vehspd_1_kph, vehspd_ecu <= vehspd_motorway_kph)
NMHCu = Inst_Mass_NMHC * np.logical_and(vehspd_ecu >= MAW_vehspd_1_kph, vehspd_ecu <= vehspd_urban_kph)
...
MAW_GHGu = np.mean(tMAW_GHG_gpkm [np.logical_and(vehspd_ecu >= MAW_vehspd_1_kph, vehspd_ecu <= vehspd_urban_kph)])
...
CO_STD_mgpmi = CO_STD_gpmi
MAW_CO_gpmi = 1.60934*MAW_CO_gpkm
CO_RDEu_mgpmi = 1.60934*CO_RDEu_mgpkm
CO_RDE_mgpmi = 1.60934*CO_RDE_mgpkm

# FTP3_CO_mgpmi = 1000* FTP3_CO_gpmi
# US06_CO_mgpmi = 1000* US06_CO_gpmi
# HwFET_CO_mgpmi = 1000* HwFET_CO_gpmi
# WLTC_CO_mgpmi = 1000* WLTC_CO_gpmi

# FTP3_CO_comp_mgpmi = 1000*FTP3_CO_comp_gpmi
# US06_CO_comp_mgpmi = 1000* US06_CO_comp_gpmi
# WLTC_CO_comp_mgpmi = 1000* WLTC_CO_comp_gpmi

FTP3_NOx_comp_mgpmi = 1000*FTP3_NOx_comp_gpmi
US06_NOx_comp_mgpmi = 1000* US06_NOx_comp_gpmi
WLTC_NOx_comp_mgpmi = 1000* WLTC_NOx_comp_gpmi
...
label_RDEu_CO = 'RDE Urban CO: ' + str(round(CO_RDEu_mgpmi, 1)) + ' mg/mile'
label_RDE_CO = 'RDE CO: ' + str(round(CO_RDE_mgpmi, 1)) + ' mg/mile'
label_CO_standards = 'CO Standards: ' + str(round(CO_STD_mgpmi, 1)) + ' g/mile'
...
Composite_NOx_mgpmi = np.array([FTP3_NOx_comp_mgpmi, US06_NOx_comp_mgpmi])
Composite_CO_gpmi =  np.array([FTP3_CO_comp_gpmi, US06_CO_comp_gpmi])  
...
if (LAB_WLTC_NOx_Fit == True) or (LAB_WLTC_NOx_Fit > 0):
    ...
    # Composite_NOx_mgpmi = 1000 * np.array([FTP3_NOx_comp_mgpmi, WLTC_NOx_comp_gpmi, US06_NOx_comp_gpmi])
...
plt.scatter(mean_GHG_gpmi, WLTC_NOx_comp_mgpmi, s=150, linewidth=3, marker="p", facecolors='none', edgecolors='m', label='WLTC Composite NOx')
...
plt.scatter(mean_vehspd_RDE, WLTC_NOx_comp_mgpmi, s=150, linewidth=3, marker="p", facecolors='none', edgecolors='r', label='WLTC Composite NOx')
