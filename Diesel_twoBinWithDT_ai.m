...
filename_udp = regexprep(vehData.filename, '_MatlabBinCalc\.xlsx.*$', '_udp.xlsx');
...
function [df, summary, windows_df, sub_summary, Invalid_count] = calculate_maw_and_subintervals(df, eCO2_g_hp_hr, pmax_hp)
    ...
    if ~isempty(idx_bin2)
        ...
        sumCO2_Bin2 = trapz(massCO2(bin2_mask));
        ...
        NOxBin2 = (trapz(massNOx(bin2_mask)) / sumCO2_Bin2) * eCO2_g_hp_hr * 1000; % mg/hp*hr
        COBin2  = (trapz(massCO(bin2_mask))     / sumCO2_Bin2) * eCO2_g_hp_hr;         % g/hp*hr
        HCBin2  = (trapz(massHC(bin2_mask))    / sumCO2_Bin2) * eCO2_g_hp_hr * 1000;  % mg/hp*hr
        PMBin2  = (trapz(massPM(bin2_mask))    / sumCO2_Bin2) * eCO2_g_hp_hr * 1000;  % mg/hp*hr
