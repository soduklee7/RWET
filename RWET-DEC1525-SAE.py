...
np.sum(Inst_Mass_NOx)*1000/ (np.sum(dVMT * np.logical_and(vehspd_ecu>=0, vehspd_ecu <= vehspd_urban_kph))/1000)
...
max_grade_pct = 5.0;
T3Bin160_NOx = 0.16;  T3Bin160_NOx_hALT = 0.16; T3Bin160_CO = 4.2;
T3Bin125_NOx = 0.125; T3Bin125_NOx_hALT = 0.16; T3Bin125_CO = 2.1;
T3Bin110_NOx = 0.110; T3Bin110_NOx_hALT = 0.16; T3Bin110_CO = 2.1;
T3Bin70_NOx = 0.110;  T3Bin70_NOx_hALT = 0.16;  T3Bin70_CO = 2.1;
T3Bin85_NOx = 0.085;  T3Bin70_NOx_hALT = 0.16;  T3Bin70_CO = 2.1;

T2Bin4_NOx = 0.04; T2Bin4_NMOG = 0.07; T2Bin4_CO = 2.1; T2Bin4_HCHO = 0.011; T2Bin4_PM = 0.01;
T2Bin5_NOx = 0.07; T2Bin5_NMOG = 0.09; T2Bin5_CO = 4.2; T2Bin5_HCHO = 0.018; T2Bin5_PM = 0.01;
...
#    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    ...
    #subplot1.set_aspect('equal')
    subplot1.plot(x1, y1, y1c, label=legend1)
    subplot1.set_xlim(0, max(x1));
    #subplot1.set_ylim(round(min(y1)), round(max(y1)+10));
    subplot1.grid();
...
def timeOBD_to_second(hms_str):
    str1 = hms_str;
    if len(str1) == 6:
        hh = str1[0:2]; mm =  str1[2:4]; ss = str1[4:6];   
    else:
	        hh = str1[0:1]; mm = str1[1:3]; ss = str1[3:5];        
...
def VehSpd_Accel_95pct(vehspd_ACCu, vehspd_ACCr, vehspd_ACCm, Vu_ACC, Vr_ACC, Vm_ACC):
    vehspd_ACCu_95pct = 0; vehspd_ACCr_95pct = 0; vehspd_ACCm_95pct = 0;
...
def Vehspd_mean(vehspd_u, vehspd_r, vehspd_m):
    vehspd_u_mean = 0; vehspd_r_mean = 0; vehspd_m_mean = 0;
...
def RDE_report (rpt, filename_pems, filename_obd, output_folder, now_hms, suffix, trip_distance, trip_duration, cold_start_duration, urban_distance, rural_distance, motorway_distance,
    ...
    rpt['EPA'][irow] = FE_mpg; irow = irow + 1;
    rpt['EPA'][irow] = CO2_gpm_test; irow = irow + 1;

    irow = 7;
    rpt['EPA'][irow] = 1.60934*CO_RDE_mgpkm; irow = irow + 1;
    rpt['EPA'][irow] = 1.60934*NOx_RDE_mgpkm; irow = irow + 1;
    rpt['EPA'][irow] = ''; irow = irow + 1;
    rpt['EPA'][irow] = 1.60934*CO_RDEu_mgpkm; irow = irow + 1;
    rpt['EPA'][irow] = 1.60934*NOx_RDEu_mgpkm; irow = irow + 1;
    ...
    irow = 70;
    rpt['EPA'][irow] = RPA_avg_EPA[0]; irow = irow + 1;
    rpt['EPA'][irow] = RPA_avg_EPA[1]; irow = irow + 1;
    rpt['EPA'][irow] = RPA_avg_EPA[2]; irow = irow + 1;
    rpt['EPA'][irow] = VA_avg_95pct_EPA[0]; irow = irow + 1;
    rpt['EPA'][irow] = VA_avg_95pct_EPA[1]; irow = irow + 1;
    rpt['EPA'][irow] = VA_avg_95pct_EPA[2]; irow = irow + 1;
...
if df['PASS-FAIL CALCULATIONS'][0] == 'CO2 window [%]': settings_MAw_CO2_pct = df['Default'][0];
if df['PASS-FAIL CALCULATIONS'][1] == 'etime_reset': etime_reset = df['Default'][1];
if df['PASS-FAIL CALCULATIONS'][2] == 'VEH_TYPE': vehicle_type = df['Default'][2];
...
WLTC_GHG_gpkm = WLTC_CO2_comp_gpkm = float(df['WLTC_CO2_gpmi'][4])/1.60934;
...
sTIME_cname = ''; sDATE_cname =''; sSTATUS_PATH_cname =''; 
iVEH_SPEED_cname = ''; imVEH_SPEED_cname = ''; iGPS_GROUND_SPEED_cname=''; iGPS_ALT_cname='';
iENG_SPEED_cname=''; iENG_LOAD_cname =''; Lambda_cname = ''; iCOOL_TEMP_cname = ''; imCOOL_TEMP_cname =''; 
TP_B_cname =''; APP_D_cname=''; APP_P_cname =''; APP_E_cname =''; P_ACCEL_POSTN_cname =''; iWfgps_cname ='';
iSCB_LAP_cname=''; Tamb_cname=''; CatalystTemp_cname='';
iCALCRT_CO2m_cname=''; iCALCRT_COm_cname = ''; iCALCRT_NOxm_cname = ''; iCALCRT_kNOxm_cname = '';
...
for i in range (npp_files):
    ...
    if i == 0:
        cols = [];
        units = [];
        icol_ECT = ''; sTIME_unit = ''; iWfgps_unit = '';
        ECT_unit = ''; iVEH_SPEED_unit =''; imVEH_SPEED_unit =''; iGPS_GROUND_SPEED_unit =''; Tamb_unit =''; Tamb_cname=''; 
        df_columns = pd.read_csv(pp_file, skiprows=1, escapechar='\\', encoding = 'unicode_escape'); # add "flot" in the SG_fuel
        ...
        try: 
            for text in columns:
                if 'sTIME' in text: 
                    ...
                    if units[icol_sTIME][0:14] == 'hh:mm:ss.xxx' or units[icol_sTIME][0:10] == 'hh:mm:ss': sTIME_unit = 'hhmmss';
                    else: sTIME_unit = 's';

                if 'iCOOL_TEMP' in text: 
                    iCOOL_TEMP_cname = text;
                    icol_ECT = columns.index(iCOOL_TEMP_cname);
                    if units[icol_ECT][0:6] == 'deg F' or units[icol_ECT][0:5] == 'degF': ECT_unit = 'deg F'; 
                    else: ECT_unit = 'deg C';

                if 'imCOOL_TEMP' in text: 
                    imCOOL_TEMP_cname = 'imCOOL_TEMP';
                    icol_ECT = columns.index(imCOOL_TEMP_cname);
                    if units[icol_ECT][0:6] == 'deg F' or units[icol_ECT][0:5] == 'degF': ECT_unit = 'deg F';
                    else: ECT_unit = 'deg C';
                ...
                if 'iWfgps' in text: 
                    ...
                    if units[icol_iWfgps][0:6] == 'gal/s': iWfgps_unit = 'gal/s';
                    else: iWfgps_unit = 'g/s';
                    
                if ('CATEMP11' or 'iCATEMP11') in text: 
                    CatalystTemp_cname = text;
                    icol_CatalystTemp = columns.index(CatalystTemp_cname);
                    if (units[icol_CatalystTemp][0:5] == 'Deg F') or (units[icol_CatalystTemp][0:5] == 'degF') or (units[icol_CatalystTemp][0:5] == 'deg F'): 
                ...
                if 'imVEH_SPEED' in text: 
                    ...
                    if units[icol_VEH_SPEED][0:4] == 'kph' or units[icol_VEH_SPEED][0:4] == 'km/h': 
                ...
                if 'iVEH_SPEED' in text: 
                    ...
                    if units[icol_VEH_SPEED][0:4] == 'mph' or units[icol_VEH_SPEED][0:4] == 'mi/h': 
                        iVEH_SPEED_unit = 'mph'; 
                    else: 
                        iVEH_SPEED_unit = 'kph';
                ...
                if 'iGPS_GROUND_SPEED' in text:
                    ...
                    if units[icol_iGPS_GROUND_SPEED][0:4] == 'mph' or units[icol_iGPS_GROUND_SPEED][0:4] == 'MPH': 
                        iGPS_GROUND_SPEED_unit = 'mph'; 
                    elif units[icol_iGPS_GROUND_SPEED][0:4] == 'kph' or units[icol_iGPS_GROUND_SPEED][0:4] == 'KPH': 
                ...
                if ('iSCB_LAT' or 'AAT' or 'iAAT' or 'AmbientTemp') in text: 
                    ...
                    if (units[icol_Tamb][0:5] == 'Deg F') or (units[icol_Tamb][0:5] == 'degF') or (units[icol_Tamb][0:5] == 'deg F'): Tamb_unit = 'deg F';
                    else: Tamb_unit = 'deg C';
        ...
        if i == 1: df2 = dft; df2_file = pp_file;
...
nindex = 0; jindex = 0; index1 = 0; index2 = 0;
...
# GHG_raw = np.zeros(nel)
# vehspd_raw = np.zeros(nel)
# nel_raw = nel;
# for i in range(nel):
#     GHG_raw[i] = float(df.iCALCRT_CO2m[i]);
#     vehspd_raw[i] = float(df.iVEH_SPEED[i]);

# GHG_urban = np.sum(GHG_raw[np.logical_and(vehspd_raw<=60, vehspd_raw >=0)])
# GHG_total = np.sum(GHG_raw);
...
if iCALCRT_CO2m_cname != '': 
    ...
    df = df.reset_index(drop = True)
# if iENG_LOAD_cname != '': 
#     df = df.drop(df[df.iENG_LOAD == ''].index);
# if iENG_SPEED_cname != '': 
#     df = df.drop(df[df.iENG_SPEED == ''].index);
#    df = df.drop(df[df.iENG_SPEED.astype(float) > 6000].index);
#df = df.drop(df[df.sDATE == 'sDATE'].index);
#df = df.drop(df[df.sDATE == 'DATE'].index);
#df = df.drop(df[df.sDATE == ''].index);
# df = df.drop(df[df.sSTATUS_PATH == 'CALIBRATION'].index);
# df = df.drop(df[df.sSTATUS_PATH == 'STANDBY'].index);
...
#     df = df.reset_index(drop = True)
#     if iVEH_SPEED_unit == 'mph': df.iVEH_SPEED = 1.60934 * df.iVEH_SPEED.astype(float);
#     else: df.iVEH_SPEED = df.imVEH_SPEED.astype(float);
#     df_US_peak_vehspd  = df[df.iVEH_SPEED > US_max_vehspd_kph];
#     if max(df.iVEH_SPEED) > EU_peak_vehspd_kph: df = df.drop(df_US_peak_vehspd.index);
#     vehspd_ecu = df.iVEH_SPEED;
# if (iGPS_GROUND_SPEED_cname != ''): 
...
#     # print('j = ', j)
#     if (vehspd_ecu[j] < 0.5 and vehspd_ecu[j-1] > 0.5) or (iENG_SPEED_cname != '' and float(df.iENG_SPEED[j]) >= 500): 
#         # print('vehspd_ecu[',j,']= ', vehspd_ecu[j])
#         # print('vehspd_ecu[',j-1,']= ', vehspd_ecu[j-1])
#         # print('df.iENG_SPEED[',j,']= ', float(df.iENG_SPEED[j]))
#         if iENG_SPEED_cname != '' and j+9 < nel and float(df.iENG_SPEED[j+9]) >= 50: 
#             vehspd_move_stop1 = j+10;    
#         else:
#             vehspd_move_stop1 = j;
#         break;   

# if vehspd_move_stop1 > 0: 
#     df = df.drop(df.index[vehspd_move_stop1:]);
#     df = df.reset_index(drop = True)
...
if iVEH_SPEED_cname != '': 
    ...
    vehspd_ecu = df.iGPS_GROUND_SPEED.astype(float);
if iENG_SPEED_cname != '': engine_RPM = df.iENG_SPEED.astype(float);
...
if OBD == TRUE:
    ...
    for i in range (len(dff_obd)):
        ...
        if i == 0:
            ...
            if i == 0: df1_obd = df_obd; df1_obd_file = obd_file;
            ...
            if i == 1: df2_obd = dft; df2_obd_file = obd_file;
    ...
    # df_ovehspd = pd.DataFrame(etime, columns=['Time_s'])
    # df_ovehspd['95%_VA_urm'] = VA_urm_95pct_EPA;
    # df_ovehspd['95%_VA_urm'] = VA_urm_95pct_EPA;
    # df_ovehspd['RPA_urm'] = RPA_avg_EPA;
    # df_ovehspd.to_excel( filename_combined + '-ovehspd.xlsx', index=False, encoding='utf-8-sig')
...
for i in range(nel):
    if iENG_SPEED_cname != '':
        # disabled for EMROAD AVL_MOVE sample initial idle duration match 
...
Vu = np.zeros(nVu); Vr = np.zeros(nVr); Vm = np.zeros(nVm);
Vuf = np.zeros(nVu); Vrf = np.zeros(nVr); Vmf = np.zeros(nVm);
Vui = np.zeros(nVu).astype(int); Vri = np.zeros(nVr).astype(int); Vmi = np.zeros(nVm).astype(int);
vehspd_accel = np.zeros(nel); dVMT = np.zeros(nel);
veh_ACC = np.zeros(nel); 

vehspd_ACCu = np.zeros(nVu); vehspd_ACCr = np.zeros(nVr); vehspd_ACCm = np.zeros(nVm)
veh_ACCu = np.zeros(nVu); Vr_ACC = np.zeros(nVr); Vm_ACC = np.zeros(nVm)
Vu_ACC = np.zeros(nVu); Vr_ACC = np.zeros(nVr); Vm_ACC = np.zeros(nVm)
dVMTu = np.zeros(nVu); dVMTr = np.zeros(nVr); dVMTm = np.zeros(nVm)
TIMEu = np.zeros(nVu); TIMEr = np.zeros(nVr); TIMEm = np.zeros(nVm)

pACCu_jel = np.zeros(nVu); pACCr_jel = np.zeros(nVr); pACCm_jel = np.zeros(nVm);
Nku_iel = np.zeros(nVu); Nkr_iel = np.zeros(nVr); Nkm_iel = np.zeros(nVm);
pACCu = np.zeros(nVu); pACCr = np.zeros(nVr); pACCm = np.zeros(nVm);

iVu = 0; iVr = 0; iVm = 0;
vehspd_145kph = 0; vehspd_100kph = 0; vehspd_zero = 0;
...
for i in range(1, nel-1):
    ...
    if vehspd_ecu[i] <=  vehspd_urban_kph and vehspd_ecu[i] >= 0: # for EMROAD vehspd > =1 and vehspd < 60 kph
        ...
        if vehspd_ecu[i] <= vehspd_1_kph/20:    # for EMROAD vehspd > =1 and vehspd < 60 kph, otherwise <= 0.2
            ...
            time_engine_idle = time_engine_idle + (etime[i] - etime[i-1])
    elif vehspd_ecu[i] > vehspd_rural_kph:
        Vm[iVm]=vehspd_ecu[i]
        dVMTm[iVm] = dVMT[i];
        TIMEm[iVm] = etime[i]
...
ehspd_ACCu_95pct = 0; vehspd_ACCr_95pct = 0; vehspd_ACCm_95pct = 0;
Vu_ACC_95pct = 0; Vr_ACC_95pct = 0; Vm_ACC_95pct = 0;
RPAu = 0; RPAr = 0; RPAm = 0; 
...
if FE_mpg == 'N/A':
    ...
    print (vehicle_title, ": ", "%0.1f" %GHG_WLTC_threshold, "gCO2 MAW, FE = ", "%0.1f" %FE_mpg, "MPG, GHG = ", label_CO2_gpm_test, ", NOx = ", label_engNOx_test);

RDE_report (rpt, filename_combined, filename_combined_obd, infile_folder, now_hms, suffix, trip_distance, trip_duration, cold_start_duration, urban_distance, rural_distance, motorway_distance,
            urban_distance_share, rural_distance_share, motorway_distance_share, urban_avg_speed, rural_avg_speed, motorway_avg_speed, total_trip_avg_speed,
            ...
            init_idle_duration, cold_start_avg_speed, cold_start_max_speed, cold_stop_time, vehspd_avg_EPA, RPA_avg_EPA, VA_avg_95pct_EPA, nVu_ACC, nVr_ACC, nVm_ACC,
...
df_columns_EPA = ['TIME', 'Vehicle Speed', 'Cumul_Distance', 'GPS Altitude', 'Engine RPM', 'Engine Load', 'Coolant Temp.', 'Ambient Temperature', 'Instantaneous Mass CO2', 
...
#plt.scatter(vehspd_avg_EPA, VA_urm_95pct_EPA, s=150, linewidth=4, marker="d", facecolors='none', edgecolors='b', label='VA_urm Stitched')
# plt.scatter(vehspd_avg_emload, VA_avg_95pct_emload, s=150, linewidth=4, marker="o", facecolors='none', edgecolors='g', label ='EMROAD Horia OBS ONE')
#plt.plot(VA_vehspd_mean_emload, VA_95pct_emload, 'b--', linewidth=3, label ='Limited EMROAD')
