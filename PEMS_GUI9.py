...
1) Create a text box with a "PEMS File" label and the "Select" button to select an Excel file. Create the "Read" button to read the selected file using df = pd.read_csv with encoding='utf-8', low_memory=False, skiprows=0' and skipping first 1 rows when clicking the "Read" button. 
...
7) Create a text box with an "OBD File" label. Create an "OBD" button" Replace the "OBD" button with a Windows 11 color large Truck icon to select a CSV file. Display the "Select an OBD file" when placing a mouse pointer on the "OBD" icon. Only Enable the "OBD File" text box and "OBD" button when the "Alignment" checkbox is True.
...
13) Create a input textbox with a "Type Alignment Values /w OBD" label. Enable only after clicking the "Check Alignment" button.
...
16) Print the "Alignment" , "Report", "Input_file_dir"  checkbox status, "Check Alignment" button, align_values_ent textbox values, and the index of 'iVEH_SPEED' in df.columns when clicking the "RUN" button.
"""
...
def make_truck_icon(size=40):
    ...
    wheel = "#111827"
    # Truck body
    d.rounded_rectangle([int(size*0.12), int(size*0.40), int(size*0.88), int(size*0.68)],
                        radius=int(size*0.10), fill=body)
    # Truck cab
    d.rounded_rectangle([int(size*0.12), int(size*0.30), int(size*0.40), int(size*0.55)],
                        radius=int(size*0.10), fill=cab)
    # Wheels
    d.ellipse([int(size*0.22), int(size*0.63), int(size*0.35), int(size*0.83)], fill=wheel)
    d.ellipse([int(size*0.65), int(size*0.63), int(size*0.78), int(size*0.83)], fill=wheel)
    return ImageTk.PhotoImage(img)
...
def read_pems_file():
    ...
    if not path:
        messagebox.showwarning("No file selected", "Please select a PEMS CSV file first.")
        return
    try:
        global df, columns_meta, vars_meta, units_meta
        # Read full CSV (skiprows=0), then set columns/vars/units from first rows and slice data
        df = pd.read_csv(path, encoding='utf-8', low_memory=False, skiprows=0)
        columns_meta = df.columns.copy()
        if len(df) > 1:
            vars_meta = df.loc[0, :]
        else:
            vars_meta = pd.Series(df.columns, index=df.columns)
        if len(df) > 2:
            units_meta = df.loc[1, :]
        else:
            units_meta = pd.Series(index=df.columns, dtype=object)
        # Update columns to vars and drop first two metadata rows
        df.columns = vars_meta.astype(str)
        if len(df) > 2:
            df = df.loc[2:, :]
        else:
            df = df.copy()
    ...
    # Update columns_listbox with df columns (now set to vars)
...
def time_alignment():
    # Mark button as clicked; enable the alignment values entry
    CheckAlignment_clicked.set(True)
    align_values_ent.configure(state="normal")
    selected_align = [align_listbox.get(i) for i in align_listbox.curselection()]
    print("time_alignment() called with:", selected_align)
    messagebox.showinfo("Check Alignment", f"Selected alignment fields:\n{selected_align}")

def run_analysis():
    # Print Entry widgets and their values
    print("pems_ent:", pems_ent, "value:", pems_file_var.get())
    print("obd_ent:", obd_ent, "value:", obd_file_var.get())

    # Print required statuses and values
    print("Alignment:", Alignment_checked.get())
    print("Report:", Report_checked.get())
    print("Input_file_dir:", Input_file_dir_checked.get())
    print("Check Alignment button clicked:", CheckAlignment_clicked.get())
    print("align_values_ent value:", align_values_var.get())

    # Index of 'iVEH_SPEED' in df.columns
    iveh_idx = -1
    if df is not None and df.columns is not None:
        cols_list = [str(c) for c in df.columns]
        try:
            iveh_idx = cols_list.index('iVEH_SPEED')
        except ValueError:
            iveh_idx = -1
    print("Index of 'iVEH_SPEED' in df.columns:", iveh_idx, "(", columns_meta[iveh_idx], ")")

    # Optional summary dialog
    ...
    summary = (
        ...
        f"Check Alignment clicked: {CheckAlignment_clicked.get()}\n"
        f"Typed Alignment Values /w OBD: {align_values_var.get()}\n"
        f"Index of 'iVEH_SPEED' in df.columns: {iveh_idx}"
    )
...
tk.Label(pems_frame, text="PEMS File", bg=ui_bg).grid(row=0, column=0, sticky="w", padx=(0, 8))
...
# Row 1: Checkbox to process all CSV files in a folder
...
# Row 2: Columns listbox with label
...
# Row 3: Blue horizontal line between two listboxes
...
# Row 4: Align listbox with label
...
# Row 5: Alignment and Report checkboxes horizontally
...
CheckAlignment_clicked = tk.BooleanVar(value=False)  # track "Check Alignment" button clicks
...
# Row 6: OBD File selector (enabled only when Alignment is True)
...
truck_icon_img = make_truck_icon(size=40) or None
if truck_icon_img:
    obd_btn = tk.Button(
        obd_frame, image=truck_icon_img, command=select_obd_file,
...
# Row 7: Input textbox for "Type Alignment Values /w OBD" (enabled only after clicking Check Alignment)
align_vals_frame = tk.Frame(container, bg=ui_bg)
align_vals_frame.grid(row=7, column=0, sticky="ew", padx=4, pady=(0, 8))
align_vals_frame.grid_columnconfigure(1, weight=1)
tk.Label(align_vals_frame, text='Type Alignment Values /w OBD', bg=ui_bg).grid(row=0, column=0, sticky="w", padx=(0, 8))
align_values_var = tk.StringVar()
align_values_ent = tk.Entry(align_vals_frame, textvariable=align_values_var, state="disabled")
align_values_ent.grid(row=0, column=1, sticky="ew")

# Row 8: Check Alignment and RUN big buttons
actions_frame = tk.Frame(container, bg=ui_bg)
actions_frame.grid(row=8, column=0, sticky="ew", padx=4, pady=(10, 0))
actions_frame.grid_columnconfigure(0, weight=1)
actions_frame.grid_columnconfigure(1, weight=1)

check_align_btn = tk.Button(
    actions_frame,
...
check_align_btn.grid(row=0, column=0, sticky="ew", padx=(0, 6), ipady=6)
